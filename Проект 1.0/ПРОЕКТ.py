...
class MyWidget(QMainWindow):
    ...
    def __init__(self):
        ...
        Del = QAction(QIcon('Del.png'), 'Del', self)
        Del.setShortcut('Ctrl+D')
        Del.setStatusTip('Del')
        Del.triggered.connect(self.delall)  
         
        
        menubartw = self.menuBar
        menubartw.addAction(Del)
        menubartw.addAction(Help)   
        self.show()
    ...
    def m(self):
        ...
        if len(writ) == 0:
            ...
            self.label.setText('Нет заметок')
        if pos!='':
            writ[last_z[0]+'    '+dt]=last_z[1]
            self.label.setText('Заметок:  ' + str(len(writ)))
            last_z=[]            
            self.listWidget.addItems([pos+'    '+dt])
            pos=''
        else:
...
class Calculator(QMainWindow):
    ...
    def append(self):
        global writ
        writ[open_z]+=' '+self.str_count.text()
        self.c=Zam_text()
        self.close()        
...
class Zam_text(QMainWindow):
    ...
    def __init__(self):
        ...
        fileMenu2.addAction(openC)
        fileMenu2.addAction(openCalendar)           
        self.show()
